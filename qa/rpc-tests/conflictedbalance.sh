...
function WaitPeers {
    while :
    do
        declare -i PEERS=$( $CMC $1 getconnectioncount )
        if (( PEERS == "$2" ))
        then
            break
        fi
...
$CMC $B2ARGS addnode 127.0.0.1:11000 onetry
WaitPeers "$B1ARGS" 1

# 2 block, 50 XBT each == 100 XBT
# These will be transactions "A" and "B"
$CMC $B1ARGS setgenerate true 2

WaitBlocks
# 100 blocks, 0 mature == 0 XBT
$CMC $B2ARGS setgenerate true 100
WaitBlocks
...
B1ADDRESS=$( $CMC $B1ARGS getnewaddress )
B2ADDRESS=$( $CMC $B2ARGS getnewaddress )

# Transaction C: send-to-self, spend A
TXID_C=$( $CMC $B1ARGS sendtoaddress $B1ADDRESS 50.0)

# Transaction D: spends B and C
TXID_D=$( $CMC $B1ARGS sendtoaddress $B2ADDRESS 100.0)

CheckBalance "$B1ARGS" 0

# Mutate TXID_C and add it to B2's memory pool:
RAWTX_C=$( $CMC $B1ARGS getrawtransaction $TXID_C )
...
MUTATEDTXID=$( $CMC $B2ARGS sendrawtransaction $MUTATEDTX_C )
