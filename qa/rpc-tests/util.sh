...
# JSON produced by commacoind, do NOT use it to try to
...
function Send {
  ...
  amount=$3
  address=$(Address $to)
  txid=$( ${SENDANDWAIT} $CMC $from sendtoaddress $address $amount )
}

# Use: Unspent <datadir> <n'th-last-unspent> <var>
function Unspent {
  local r=$( $CMC $1 listunspent | awk -F'[ |:,"]+' "\$2 ~ /$3/ { print \$3 }" | tail -n $2 | head -n 1)
  echo $r
}
...
function CreateTxn1 {
  ...
  RAWTXN=$( $CMC $1 createrawtransaction "[{\"txid\":\"$TXID\",\"vout\":$VOUT}]" "{\"$3\":$AMOUNT}")
  ExtractKey hex "$( $CMC $1 signrawtransaction $RAWTXN )"
}

# Use: SendRawTxn <datadir> <hex_txn_data>
function SendRawTxn {
  ${SENDANDWAIT} $CMC $1 sendrawtransaction $2
}
