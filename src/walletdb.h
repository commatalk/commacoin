...
// Copyright (c) 2016-2018 The COMMACOIN developers
...
public:
    ...
    bool WriteZCMCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZCMCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZCMCSeed_deprecated(uint256& seed);
    bool EraseZCMCSeed();
    bool EraseZCMCSeed_deprecated();

    bool WriteZCMCCount(const uint32_t& nCount);
    bool ReadZCMCCount(uint32_t& nCount);
