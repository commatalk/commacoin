...
CzCMCWallet::CzCMCWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCMCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCMCSeed_deprecated()) {
                    LogPrintf("%s: Updated zCMC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzCMCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCMCCount(nCountLastUsed);
    else if (!walletdb.ReadZCMCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCMCWallet::Lock()
{
    seedMaster = 0;
}

void CzCMCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCMCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCMC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCMCWallet::LoadMintPoolFromDB()
{
...
void CzCMCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCMCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCMCWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzCMCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCMC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzCMCWallet::SeedToZCMC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCMCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCMCWallet::GenerateDeterministicZCMC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCMCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCMCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
