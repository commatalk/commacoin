...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or commacoin.conf setting gen)\n"
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CMCENT_NOT_CONNECTED, "COMMACOIN is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CMCENT_IN_INITIAL_DOWNLOAD, "COMMACOIN is downloading blocks...");
    ...
    if (!lpval.isNull()) {
        ...
        if (!IsRPCRunning())
            throw JSONRPCError(RPC_CMCENT_NOT_CONNECTED, "Shutting down");
