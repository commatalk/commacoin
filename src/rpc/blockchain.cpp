...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    result.push_back(Pair("zCMCsupply", zpivObj));
...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in commacoin\n"
...
UniValue gettxout(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of commacoin addresses\n"
            "     \"commacoinaddress\"   	 	(string) commacoin address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
...
UniValue getfeeinfo(const UniValue& params, bool fHelp)
{
    ...
    for (int i = nStartHeight; i <= nBestHeight; i++) {
        ...
        for (const CTransaction& tx : block.vtx) {
            ...
            nBytes += tx.GetSerializeSize(SER_NETWORK, CMCENT_VERSION);
            nTotal++;
        }
