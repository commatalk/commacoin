...
#ifndef COMMACOIN_ZCMCWALLET_H
#define COMMACOIN_ZCMCWALLET_H
...
public:
    CzCMCWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZCMC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZCMC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //COMMACOIN_ZCMCWALLET_H
