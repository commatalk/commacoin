...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 CMC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zCMC send
enum ZerocoinSpendStatus {
    ZCMC_SPEND_OKAY = 0,                            // No error
    ZCMC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZCMC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZCMC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZCMC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZCMC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZCMC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZCMC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZCMC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZCMC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZCMC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZCMC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZCMC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZCMC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZCMC_SPENT_USED_ZCMC = 14,                      // Coin has already been spend
    ZCMC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZCMC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZCMCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzCMCWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzCMCTracker> zpivTracker;
    ...
    void setZWallet(CzCMCWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzCMCTracker>(new CzCMCTracker(strWalletFile));
    }

    CzCMCWallet* getZWallet() { return zwalletMain; }
    ...
    /** zCMC reset */
    boost::signals2::signal<void()> NotifyzCMCReset;
