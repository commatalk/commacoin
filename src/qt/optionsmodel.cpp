...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::CMC);
    ...
    if (!settings.contains("nAnonymizeCivxAmount"))
        settings.setValue("nAnonymizeCivxAmount", 1000);

    nAnonymizeCivxAmount = settings.value("nAnonymizeCivxAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeCivxAmount"))
        SoftSetArg("-anonymizecommacoinamount", settings.value("nAnonymizeCivxAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in commacoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCivxAmount:
            return QVariant(nAnonymizeCivxAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCivxAmount:
            nAnonymizeCivxAmount = value.toInt();
            settings.setValue("nAnonymizeCivxAmount", nAnonymizeCivxAmount);
            emit anonymizeCivxAmountChanged(nAnonymizeCivxAmount);
            break;
        case CoinControlFeatures:
