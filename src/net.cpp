...
void CNode::PushVersion()
{
    ...
    PushMessage("version", PROTOCOL_VERSION, nLocalServices, nTime, addrYou, addrMe,
        nLocalHostNonce, FormatSubVersion(CMCENT_NAME, CMCENT_VERSION, std::vector<string>()), nBestHeight, true);
}
...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (COMMACOIN users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "COMMACOIN " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. COMMACOIN Core is probably already running."), addrBind.ToString());
        else
...
bool CAddrDB::Write(const CAddrMan& addr)
{
    ...
    CDataStream ssPeers(SER_DISK, CMCENT_VERSION);
    ...
    CAutoFile fileout(file, SER_DISK, CMCENT_VERSION);
    if (fileout.IsNull())
...
bool CAddrDB::Read(CAddrMan& addr)
{
    ...
    CAutoFile filein(file, SER_DISK, CMCENT_VERSION);
    if (filein.IsNull())
        return error("%s : Failed to open file %s", __func__, pathAddr.string());
    ...
    CDataStream ssPeers(vchData, SER_DISK, CMCENT_VERSION);
...
bool CBanDB::Write(const banmap_t& banSet)
{
    ...
    CDataStream ssBanlist(SER_DISK, CMCENT_VERSION);
...
bool CBanDB::Read(banmap_t& banSet)
{
    ...
    CDataStream ssBanlist(vchData, SER_DISK, CMCENT_VERSION);
