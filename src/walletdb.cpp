...
void CWalletDB::ListAccountCreditDebit(const string& strAccount, list<CAccountingEntry>& entries)
{
    ...
    while (true) {
        // Read next record
        CDataStream ssKey(SER_DISK, CMCENT_VERSION);
        if (fFlags == DB_SET_RANGE)
            ssKey << std::make_pair(std::string("acentry"), std::make_pair((fAllAccounts ? string("") : strAccount), uint64_t(0)));
        CDataStream ssValue(SER_DISK, CMCENT_VERSION);
...
DBErrors CWalletDB::LoadWallet(CWallet* pwallet)
{
    ...
    if (wss.nFileVersion < CMCENT_VERSION) // Update
        WriteVersion(CMCENT_VERSION);
...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    ...
    BOOST_FOREACH (CDBEnv::KeyValPair& row, salvagedData) {
        if (fOnlyKeys) {
            CDataStream ssKey(row.first, SER_DISK, CMCENT_VERSION);
            CDataStream ssValue(row.second, SER_DISK, CMCENT_VERSION);
            string strType, strErr;
...
bool CWalletDB::WriteZCMCSeed(const uint256& hashSeed, const vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseZCMCSeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteZCMCSeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseZCMCSeed_deprecated()
{
    return Erase(string("dzs"));
}

bool CWalletDB::ReadZCMCSeed(const uint256& hashSeed, vector<unsigned char>& seed)
{
    return Read(make_pair(string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadZCMCSeed_deprecated(uint256& seed)
{
    return Read(string("dzs"), seed);
}

bool CWalletDB::WriteZCMCCount(const uint32_t& nCount)
{
    return Write(string("dzc"), nCount);
}

bool CWalletDB::ReadZCMCCount(uint32_t& nCount)
{
