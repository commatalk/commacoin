...
#if defined(NDEBUG)
#error "COMMACOIN cannot be compiled without assertions."
#endif
...
{
    //Check to see if the zCMC is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zCMC spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zCMC without the correct spend type. txid=%s", __func__,
                tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zCMC spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zCMC spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if (tx.IsZerocoinSpend()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zCMC spend tx %s already in block %d",
            ...
            // Check that zCMC mints are not already known
    ...
    CBlockIndex* pindexSlow = NULL;
    {
        ...
        if (fTxIndex) {
            ...
            if (pblocktree->ReadTxIndex(hash, postx)) {
                CAutoFile file(OpenBlockFile(postx, true), SER_DISK, CMCENT_VERSION);
                if (file.IsNull())
...
bool WriteBlockToDisk(CBlock& block, CDiskBlockPos& pos)
{
    ...
    CAutoFile fileout(OpenBlockFile(pos), SER_DISK, CMCENT_VERSION);
    if (fileout.IsNull())
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos)
{
    ...
    CAutoFile filein(OpenBlockFile(pos, true), SER_DISK, CMCENT_VERSION);
    if (filein.IsNull())
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight >= 0 && nHeight < Params().LAST_POW_BLOCK()) {
        nSubsidy = 4047565 * COIN;
    } else if (nHeight >= Params().LAST_POW_BLOCK() && nHeight <= 21600) {
        nSubsidy = 1 * COIN;
    } else if (nHeight >= 21601 && nHeight <= 151201) {
        nSubsidy = 2000 * COIN;
    } else if (nHeight >= 151202 && nHeight <= 410401) {
        nSubsidy = 1000 * COIN;
    } else if (nHeight >= 410402 && nHeight <= 1058402) {
        nSubsidy = 500 * COIN;
    } else if (nHeight >= 1058403 && nHeight <= 3132003) {
        nSubsidy = 300 * COIN;
    } else if (nHeight >= 3132004 && nHeight <= 7279204) {
        nSubsidy = 200 * COIN;
    } else if (nHeight >= 7279205 && nHeight <= 20757605) {
        nSubsidy = 140 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZCMCStake)
{
    int64_t ret = 0;
    ...
    if (nHeight <= 43200) {
        ...
        //When zCMC is staked, masternode only gets 2 CMC
        ret = 3 * COIN;
        if (isZCMCStake)
            ret = 2 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from COMMACOIN
...
{
    RenameThread("commacoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZCMCMinted()
{
...
void RecalculateZCMCSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zCMC supply
...
bool RecalculateCMCSupply(int nHeightStart)
{
...
bool UpdateZCMCSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        pos.nTxOffset += ::GetSerializeSize(tx, SER_DISK, CMCENT_VERSION);
    }
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateCMCSupply(Params().Zerocoin_StartHeight());
    }

    //Track zCMC money supply in the block index
    if (!UpdateZCMCSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zCMC supply for block=%s height=%d", __func__, block.GetHash().GetHex(), pindex->nHeight), REJECT_INVALID);
    ...
    if (pindex->GetUndoPos().IsNull() || !pindex->IsValid(BLOCK_VALID_SCRIPTS)) {
        if (pindex->GetUndoPos().IsNull()) {
            CDiskBlockPos pos;
            if (!FindUndoPos(state, pindex->nFile, pos, ::GetSerializeSize(blockundo, SER_DISK, CMCENT_VERSION) + 40))
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert CMC to zCMC
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zCMC spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zCMC serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zCMC stake block is less than allowed start height", __func__);

    if (CZPivStake* zCMC = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zCMC->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zCMC stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zCMC stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zCMC->GetValue()));
        if (nChecksum200 != zCMC->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zCMC->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZCMC() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zCMC fails context checks", __func__));
    ...
    try {
        unsigned int nBlockSize = ::GetSerializeSize(block, SER_DISK, CMCENT_VERSION);
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zCMC mints and %d zCMC spends\n", __func__, nMints, nSpends);
    ...
    LogPrintf("%s : ACCEPTED Block %ld in %ld milliseconds with size=%d\n", __func__, GetHeight(), GetTimeMillis() - nStartTime,
        pblock->GetSerializeSize(SER_DISK, CMCENT_VERSION));
...
bool static LoadBlockIndexDB(string& strError)
{
    ...
    for (std::set<int>::iterator it = setBlkDataFiles.begin(); it != setBlkDataFiles.end(); it++) {
        ...
        if (CAutoFile(OpenBlockFile(pos, true), SER_DISK, CMCENT_VERSION).IsNull()) {
            return false;
        }
    }
...
bool LoadExternalBlockFile(FILE* fileIn, CDiskBlockPos* dbp)
{
    ...
    try {
        ...
        CBufferedFile blkdat(fileIn, 2 * MAX_BLOCK_SIZE_CURRENT, MAX_BLOCK_SIZE_CURRENT + 8, SER_DISK, CMCENT_VERSION);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // COMMACOIN: We use certain sporks during IBD, so check to see if they are
...
{
    // Open history file to append
    CAutoFile fileout(OpenUndoFile(pos), SER_DISK, CMCENT_VERSION);
    if (fileout.IsNull())
...
{
    // Open history file to read
    CAutoFile filein(OpenUndoFile(pos, true), SER_DISK, CMCENT_VERSION);
    if (filein.IsNull())
