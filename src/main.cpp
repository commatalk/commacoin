...
#if defined(NDEBUG)
#error "Comma cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uCMC) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight <= Params().LAST_POW_BLOCK())
        nSubsidy = 21749998 * COIN; // premine
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // CMC: We use certain sporks during IBD, so check to see if they are
